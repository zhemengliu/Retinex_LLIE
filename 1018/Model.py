...
# from three_stage import ThreeStageModel
...
class LLIE(nn.Module):
    def __init__(self, args):
        ...
        # self.denoise_net = ThreeStageModel()


    def forward(self, low_img):
        low_R, low_L = self.decom_net(low_img)
        low_R = torch.where(torch.isnan(low_R), torch.tensor(1-1e-5, device=low_R.device), low_R)

        #assert not torch.isnan(low_R).any(), "low_R contains NaN"
        with torch.no_grad():
        ...
        # print(f"low_R shape: {low_R.shape}")
