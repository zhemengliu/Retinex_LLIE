...
class LLIE(nn.Module):
    ...
    def gamma_correction(self, img):
        """改进的gamma校正"""
        # 使用更温和的gamma范围
        gamma = random.uniform(1.1, 1.5)
        corrected = torch.pow(img, 1.0 / gamma)
    ...
    def forward(self, x_low):
        ...
        # 分解
        low_R, low_L = self.decom_net(x_low)

        # gamma校正
        x_gamma = self.gamma_correction(x_low)
...
if __name__== '__main__':
    ...
    net=LLIE(args).cuda()
    low_R, low_L, gamma_R, gamma_L, enhance_L1, enhance_L, x_gamma, enhance_img=net(x)
