...
def compute_total_loss(x_low, low_R, low_L, gamma_R, gamma_L, x_gamma, enhance_L, enhance_img):
    ...
    total_decom_loss = decom_recon_loss + 10*decom_consist_loss + decom_tv_loss + 0.1 * L_con
    print(f'decom_recon_loss:{decom_recon_loss:.4f}, decom_consist_loss: {10*decom_consist_loss:.4f}, decom_tv_loss:{decom_tv_loss:.4f}, L_con:{0.1 * L_con:.4f}')
    ...
    total_illum_loss = exp_loss_L  #+ 0.01 * tv_loss #+ exp_loss_L+ 0.3 * color_loss

    # 总损失（关键修改：确保所有损失项都是标量，并最终求和为标量）
    total_loss = (10 * total_decom_loss + total_illum_loss)
