...
def compute_total_loss(x_low, low_R, low_L, gamma_R, gamma_L, x_gamma, enhance_L, enhance_img):
    ...
    exp_loss = L_exp(patch_size=16, mean_val=0.9)(enhance_L)
    exp_R_loss = L_exp(patch_size=16, mean_val=0.4)(low_R) + torch.abs(torch.mean(low_R) - 0.4)

    total_illum_loss = exp_loss + 1.5 * exp_R_loss #+ 50 * AAA #+ 0.01 * tv_loss #+ exp_loss_L+ 0.3 * color_loss
