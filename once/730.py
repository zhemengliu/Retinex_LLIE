...
class SCurveEnhanceNet(nn.Module):
    def __init__(self, num_steps=2):  # 减少步数，避免过度压缩
        super().__init__()
    ...
    def forward(self, L):
        ...
        for i in range(self.num_steps):
            ...
            # 调整参数范围：避免输出过暗
            a = torch.sigmoid(a) * 0.5 + 1.0  # 缩放因子 a ∈ [1.0, 2.0]（确保亮度提升）
            b = torch.sigmoid(b) * 1 + 1  # 曲率因子 b ∈ [1.0, 4.0]（曲线更平缓）
            c = torch.tanh(c) * 0.2  # 偏移因子 c ∈ [-0.3, 0.3]（减少偏移幅度）
