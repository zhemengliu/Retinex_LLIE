...
# 检查点保存目录
CHECKPOINT_DIR = "D:\\文件\\大创\\train730"
...
DENOISE_LR = 1e-4
DENOISE_EPOCHS = 10
NOISE_MODEL = ('gaussian', 50)  # 噪声模型：高斯噪声，标准差
...
class SCurveEnhanceNet(nn.Module):
    def __init__(self, num_steps=4):
        ...
        self.e_conv7 = nn.Conv2d(number_f * 2, 3 * num_steps, 3, 1, 1, bias=True)
    ...
    def forward(self, L):
        ...
        for i in range(self.num_steps):
            ...
            # 调整参数范围，避免照明分量过亮
            a = torch.sigmoid(a) * 0.3 + 0.2  # 振幅 ∈ [0.3, 0.7]，避免过强
            b = torch.sigmoid(b) * 1.5 + 0.5  # 陡峭度 ∈ [0.5, 2.0]，避免过度调整
            c = torch.tanh(c) * 0.15  # 平移量 ∈ [-0.15, 0.15]，微调偏移

            if self.debug:
                print(
                    f"第{i + 1}步 - a: [{a.min():.3f},{a.max():.3f}], b: [{b.min():.3f},{b.max():.3f}], c: [{c.min():.3f},{c.max():.3f}]")

            # 应用S曲线: f(x) = a / (1 + exp(-b*(x - c)))
            current_L = a / (1 + torch.exp(-b * (current_L - c)))
            current_L = torch.clamp(current_L, 0.1, 0.9)  # 限制范围，避免全白

        return current_L


# 修复的空间损失函数（删除未使用变量，修复设备问题）
class L_spa(nn.Module):
    def __init__(self):
        super(L_spa, self).__init__()
        # 使用设备无关的张量初始化
        kernel_left = torch.tensor([[0, 0, 0], [-1, 1, 0], [0, 0, 0]], dtype=torch.float32, device=device).unsqueeze(
            0).unsqueeze(0)
        kernel_right = torch.tensor([[0, 0, 0], [0, 1, -1], [0, 0, 0]], dtype=torch.float32, device=device).unsqueeze(
            0).unsqueeze(0)
        kernel_up = torch.tensor([[0, -1, 0], [0, 1, 0], [0, 0, 0]], dtype=torch.float32, device=device).unsqueeze(
            0).unsqueeze(0)
        kernel_down = torch.tensor([[0, 0, 0], [0, 1, 0], [0, -1, 0]], dtype=torch.float32, device=device).unsqueeze(
            0).unsqueeze(0)
    ...
    def forward(self, org, enhance):
        ...
        weight_diff = torch.max(
            torch.tensor(1.0, device=device) + 10000 * torch.min(org_pool - torch.tensor(0.3, device=device),
        ...
        # 加权计算梯度损失
        D_left = torch.pow(D_org_letf - D_enhance_letf, 2) * weight_diff
        D_right = torch.pow(D_org_right - D_enhance_right, 2) * weight_diff
        D_up = torch.pow(D_org_up - D_enhance_up, 2) * weight_diff
        D_down = torch.pow(D_org_down - D_enhance_down, 2) * weight_diff
...
class L_exp(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = self.pool(x)
        # 使用设备无关的张量
        d = torch.mean(torch.pow(mean - torch.tensor(self.mean_val, device=x.device), 2))
        return d
