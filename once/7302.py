...
class SCurveEnhanceNet(nn.Module):
    def __init__(self, num_steps=4):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')  # 更适合ReLU的初始化
    ...
    def forward(self, L):
        ...
        for i in range(1):
            ...
            # 约束参数范围确保亮度提升
            a = torch.sigmoid(a) * 1.5 + 0.5  # 放大因子 [0.5, 2.0]
            b = torch.sigmoid(b) * 2 + 0.5  # 曲率因子 [0.5, 2.5]
            c = torch.tanh(c) * 0.5  # 偏移量 [-0.5, 0.5]
            ...
            current_L = torch.clamp(current_L, 0.0, 1.0)
